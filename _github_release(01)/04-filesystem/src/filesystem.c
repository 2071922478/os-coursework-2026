...
#define INODES_PER_BLOCK (BLOCK_SIZE / sizeof(Inode))   /* 512/32 = 16 */
...
static int disk_read(int blkno, void *buf) {
    fseek(g_disk, blkno * BLOCK_SIZE, SEEK_SET);
    return (int)fread(buf, 1, BLOCK_SIZE, g_disk);
}
...
/* 获取文件数据块 (必要时分配) */
static int get_data_block(int ino, Inode *inode, int blk_idx, bool alloc) {
...
static void cmd_write(const char *path, const char *data) {
    ...
    while (written < datalen) {
        int blk = get_data_block(ino, &inode, blk_idx, true);
...
static void cmd_cat(const char *path) {
    ...
    for (int i = 0; i < nblocks; i++) {
        int blk = get_data_block(ino, &inode, i, false);
        if (blk <= 0) break;
        disk_read(blk, buf);
